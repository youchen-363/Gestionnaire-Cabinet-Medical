...
<?php
    ...
    $medSelected = '';
    if (!empty($_POST['nomMedecin'])){
        $medSelected = $_POST['nomMedecin'];
        $idM = getIdMedecinByNomPrenom($medSelected);
        $cons = getConsultationsParMedecin($idM);
    }

    $idM = $dateHeure = "";
    // verifier si le bouton supprimer est clique
    if (isset($_POST['id_medecin']) && isset($_POST['dateHeure'])){
        $idM = $_POST['id_medecin'];
        $dateHeure = $_POST['dateHeure'];
        $ins = supprimerConsultation($idM, $dateHeure);
        if ($ins){
            $cons = getAllConsultations();
        } else {
            echo "Suppression consultation echec";
        }
    }
...
<head>
    ...
    <title>Consultations - Cabi.Net</title>
    <link rel="shortcut icon" type="image/png" href="../img/cabinet.png"/>
</head>
<body class="d-flex flex-column justify-content-between">
    <?php
        setHeader('consultation.php', $_SESSION['user']);
    ?>
    ...
    <main class="flex-grow-1">
        <div class="d-flex flex-column align-items-center mx-auto usager-content justify-content-start" style="width: 65%; height: 100%;">
            <div class="d-flex flex-row justify-content-between my-5" style="width: 85%;">
                    <form method="post" action="consultation.php" id="search_form" style="width: 50%;">
                        <input type="search" name = "nomMedecin" id="nomMedecin" value="<?php echo htmlspecialchars("$medSelected") ?>"  placeholder="Recherchez une consultation par médecin" class="rounded-pill border border-0 p-2 ps-4" style="width: 100%;" >
                    </form>
                    <a class="btn btn-secondary pe-4 rounded-pill" href="consultation_saisie_usager.php">
                ...
                <?php 
                    if (count($cons) > 0){
                        ...
                        <div class="accordionHeader d-flex flex-row justify-content-between align-items-center">
                            <h5 class="my-2 ms-4">Médecin</h5>
                            <h5 class="mb-0">Usager</h5>
                        ...
                        ';
                        foreach ($cons as $con){
                            $medecin = getMedecinById($con['idMedecin']);
                            $usager = getUsagerById($con['idUsager']);
                            $consultation = new Consultation($medecin['nom'], $medecin['prenom'], $medecin['idMedecin'], $usager['nom'], $usager['prenom'], $usager['idUsager'], $con['DateHeure'], $con['Duree']);
                        ...
                        '<div class="d-flex flex-row align-items-center justify-content-center" style="height:100px; width:100%; border-radius: 15px; border: 1px solid #D25050;background: #D25050;">
                            ...
                            <h6 style="color:#FFF;; margin-left: 10%; width: 50%;">Aucune consulation pour le médecin <span style="font-weight:bold;">'.$medSelected.'</span> n\'a été trouvée. Veuillez vérifier que ce médecin est enregistré dans votre cabinet, autrement veuillez enregistrer une consultation pour ce dernier.</h5>
                        </div>';
                    } else {
                        echo 
                        '<div class="d-flex flex-row align-items-center justify-content-center" style="height:100px; width:100%; border-radius: 15px; border: 1px solid #3EB1A8;background: #3EB1A8;">
                            ...
                            <h6 style="color:#FFF;; margin-left: 10%; width: 50%;">Il n\'y a pas de consultation enregistrée dans votre cabinet.</h5>
                        </div>';
                    } 
                ?>
            </div>
    ...
    <script>
        document.getElementById('nomMedecin').addEventListener('keyup', function (e) {
            if (e.keyCode === 13) {
