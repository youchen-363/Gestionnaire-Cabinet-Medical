...
<body class="d-flex flex-column justify-content-between">
    <?php
        setHeader('consultation_saisie_usager.php',$_SESSION['user']);
    ?>
    ...
    <main class="flex-grow-1">
        ...
        <h2 class="mt-5 mb-3 fw-bold">ETAPE 1 : Choix de l'usager</h2>
            <div class="d-flex flex-row justify-content-center my-5" style="width: 85%;">
                <form method="post" action="consultation_saisie_usager.php" id="search_form" style="width: 100%;">
            ...
            <div style="width: 85%;">
                <?php 
                    if (count($usgs) > 0){
                        echo '
                        <div class="accordionHeader d-flex flex-row align-items-center">
                            <h5 class="my-2 ms-4 w-50">Usager</h5>
                            <h5 class="mb-0 w-50">Médecin référent</h5>
                        </div>
                        ...
                        foreach ($usgs as $row){
                            ...
                            $usager->afficherInfosPourConsultation();
                        }
                        echo'</div>';
                    } else if ($motcle != ""){
                        echo 
    ...
    <script>
        var tousLesLiens = document.getElementsByClassName('usager-consultation-row');
        // Ajout d'un gestionnaire d'événement à chaque élément
        for (let ligne of tousLesLiens) {
            ligne.addEventListener('mouseover', function() {
                // Ajout de la classe "hovered" au survol de chaque élément
                ligne.style.backgroundColor = 'var(--light-blue)';
            });

            ligne.addEventListener('mouseout', function() {
                // Retrait de la classe "hovered" à la fin du survol de chaque élément
