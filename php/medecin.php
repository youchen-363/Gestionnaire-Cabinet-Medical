...
<?php
    ...
    if (isset($_POST['nomMed'])){
         $nomMed = $_POST['nomMed'];
        $meds = getMedecinByNom($nomMed);
    }

    // supprimer medecin par id 
    if (isset($_POST['idM'])) {
        $idM = $_POST['idM'];  
        $ins = deleteMedecinParId($idM);
        if ($ins === true){
            $meds = getMedecinByNom($nomMed);
        } else {
            echo "Supression echec";
        }
    }
?>
...
<body class="d-flex flex-column justify-content-between">
    ...
    <main class="flex-grow-1">
        <div class="d-flex flex-column align-items-center mx-auto usager-content justify-content-start" style="width: 65%; height: 100%;">
            <div class="d-flex flex-row justify-content-between my-5" style="width: 85%;">
                <form method="post" action="medecin.php" id="search_form" style="width: 50%;">
                    <input type="search" name = "nomMed" id="nomMed" value="<?php echo htmlspecialchars("$nomMed") ?>"  placeholder="Recherchez un médecin par nom, prénom" class="rounded-pill border border-0 p-2 ps-4" style="width: 100%;" >
                </form>
                <a class="btn btn-secondary pe-4 rounded-pill" href="medecin_saisie.php">
            ...
            if (count($meds) > 0) {
                ...
                foreach ($meds as $row ){
                    $medecin = new Medecin($row['idMedecin'], $row['nom'], $row['prenom'], $row['civilite'] );
                ...
                '<div class="d-flex flex-row align-items-center justify-content-center" style="height:100px; width:100%; border-radius: 15px; border: 1px solid #D25050;background: #D25050;">
                    ...
                    <h6 style="color:#FFF;; margin-left: 10%; width: 50%;">Aucun médecin ayant pour nom ou prénom <span style="font-weight:bold;">'.$_POST['nomMed'].'</span> n\'a été trouvé</h5>
                </div>';
            } else {
                echo 
                '<div class="d-flex flex-row align-items-center justify-content-center" style="height:100px; width:100%; border-radius: 15px; border: 1px solid #3EB1A8;background: #3EB1A8;">
                    ...
                    <h6 style="color:#FFF;; margin-left: 10%; width: 50%;">Il n\'y a pas de médecin inscrit dans votre cabinet, veuillez en ajouter afin de pouvoir enregistrer vos futures consultations</h5>
                </div>';
            } ?>
            </div>
        </div>
    ...
    <script>
        document.getElementById('nomMed').addEventListener('keyup', function (e) {
            if (e.keyCode === 13) {
